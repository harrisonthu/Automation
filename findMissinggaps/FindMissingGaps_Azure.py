...
### Client:     T Rowe Price (sqlsv-wm-us-prod.database.windows.net\SQLDB_TRP_US) ###
### Description:
### Finding the missing data gaps in the three tables names below on Azure platform ###
...
def getAdobeTable():
  ...
  getAdobeTable.now = time.time()  # Time after it finished
  ...
  #print("Finished finding missing data gaps. It took: ",int(getAdobeTable.now-getAdobeTable.then), " seconds\n")
...
def getSizmekTable():
  ...
  getSizmekTable.now = time.time()  # Time after it finished
  ...
  #print("Finished finding missing data gaps. It took: ",int(getSizmekTable.now-getSizmekTable.then), " seconds")
...
def main():
  ...
  if (getIASTable().empty):
    ...
    print("Max date for Table [DFID052284_DS999622_FTP_IAS_TRP_Extracted]: ", getIASTable.maxdateIAS.iloc[0,:].to_string())
    print("\n")
  else:
  ...
  if (getAdobeTable().empty):
    ...
    print("Max date for Table [DFID052226_DS999621_Email_Omniture_TRP_US_Extracted]: ", getAdobeTable.maxdateAdobe.iloc[0,:].to_string())
  else:
